...
__weak void Int_ModBus_GetREG_HOLD_BUF(uint16_t buffers[])
{
}

//上报的数据：当前角度（float）,电机的状态 1：运行 0：停止
...
//向输入寄存器写入当前角度
void Int_ModBus_SetCurrentAngle(float angle)
{
    memcpy(REG_INPUT_BUF,&angle,4);
}

void Int_ModBus_SetMotorStatus(uint8_t motorStatus)
{
    REG_DISC_BUF[2] = motorStatus;
}
...
eMBErrorCode eMBRegHoldingCB(UCHAR *pucRegBuffer, USHORT usAddress, USHORT usNRegs, eMBRegisterMode eMode)
{
    ...
    if (eMode == MB_REG_WRITE)
    {
        ...
        // modbus协议,已经将收到主设备的信息,写入保持就寄存器数组当中
        // 什么时候会执行,只要从设备接收到数据,将数据写入数组当中,此函数就会执行一次
        Int_ModBus_GetREG_HOLD_BUF(REG_HOLD_BUF);
    }
    // 读寄存器
    else
    {
