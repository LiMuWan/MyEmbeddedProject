...
void App_Motor_GateWayConsole_GetDataByBuffer(uint8_t buffer[]);
//上报数据
void App_Motor_GateWayConsole_UploadData(void);

//接收Modbus任务
void App_GateWayToConsole_Task1(void *params)
{
...
void App_GateWayToConsole_Task(void *params)
{
    while (1)
    {
        ...
        if (res == pdTRUE)
        {
            ...
            {
                //1.CAN持续接收总线数据
                Can_Receive_T gateway_receive_buffers[3] = {0};
                uint8_t sizes = 0;
                Int_CAN_ReceiveData(gateway_receive_buffers,&sizes);

                for(uint8_t i = 0;i<sizes;i++)
                {
                    //整理数据
                    App_Motor_GateWayConsole_GetDataByBuffer(gateway_receive_buffers[i].data);
...
void App_Motor_GateWayConsole_GetDataByBuffer(uint8_t buffer[])
{
    memcpy(&current_angle,buffer,4);
    motor_status = buffer[4];
}
